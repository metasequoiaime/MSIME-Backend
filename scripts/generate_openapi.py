...
dictionary_ops=[
 ('/v1/users/me/dictionary/candidates','post','查询个人词库与基础词库合并候选',obj({'text':string(),'kind':string(enum=['pinyin','wubi','english','quick','jianpin'],default='pinyin'),'scheme':string(enum=['pinyin','shuangpin','wubi'],default='pinyin'),'profile':string(enum=['xiaohe','ziranma','shoudao','microsoft'],default='xiaohe'),'limit':{'type':'integer','minimum':1,'maximum':200,'default':20}},['text'],True),obj(dict(candidate_response['properties'],revision={'type':'integer','format':'int64'})),200,'同一数据库快照读取当前用户覆盖及 revision，再由 Engine 将覆盖和删除记录回放到临时词库副本。只读公共基础词库；副本在查询完成或失败后清理；返回完整词库版本以供客户端判断缓存。'),
