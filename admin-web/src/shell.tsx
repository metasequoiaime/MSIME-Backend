...
export function Shell() {
  ...
  return <div><aside><Link className="brand" to="/"><span className="brand-icon">杉</span><div>水杉{session?.version && <span className="brand-version" title={`服务版本 ${session.version}`}>v{session.version}</span>}<small>管理控制台</small></div></Link><p className="nav-label">工作空间</p>
    <nav>{Object.entries(pages).filter(([key]) => key !== "admins" || session?.can_manage_admins).map(([key, [title, , icon]]) => key === "overview" ? <Link key={key} to="/" className={path === "/" ? "active" : ""} aria-current={path === "/" ? "page" : undefined}><Icon className="nav-icon" name={icon} /><span>{title}</span></Link> : <Link key={key} to="/$section" params={{ section: key }} className={path === `/${key}` ? "active" : ""} aria-current={path === `/${key}` ? "page" : undefined}><Icon className="nav-icon" name={icon} /><span>{title}</span></Link>)}</nav>
