...
3. 设置独立的随机管理员密钥，例如用 `openssl rand -hex 32` 生成，保存到部署环境的 `MSIME_ADMIN_TOKEN`。不可复用客户端令牌。不要把管理员密钥放进前端源码、客户端安装包或版本库。
4. 使用新二进制先执行迁移：`./msime-server -config /config/config.json -migrate-users`。镜像中可在正常入口后追加 `-migrate-users`。迁移是幂等的；后台启用但缺少表时服务会拒绝启动。
5. 正常启动镜像；容器中的 `listen` 应为 `0.0.0.0:8080`。配置 `admin.msime.app` 的 DNS 指向入口，并在入口终止 HTTPS，将该域名的请求转发到相同的 Go 端口，保留原始 Host。Go 不信任 `X-Forwarded-Host`。
...
浏览器访问 `https://admin.msime.app` 后输入管理员密钥。密钥只保留在页面内存；刷新、关闭或退出后需重新输入。所有后台 `/api/*` 均校验独立 Bearer 密钥，普通用户/设备令牌无权访问。后台域名不承载 `/v1/*` 客户端 API。默认 `admin.enabled: false`，不会改变已有域名路由。

当前为单管理员共享密钥模式，审计记录操作、目标与时间，不区分个人管理员身份。更换密钥并重启所有副本即可撤销旧密钥。
