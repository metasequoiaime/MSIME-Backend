...
| 用户候选调频、置顶、固定位置、删除与升级回放 | 保存用户覆盖、固定位置和删除记录；支持跨端导出与恢复，不修改全局基础词库 | 固定位置 GET/PUT/DELETE、版本冲突、全拼/双拼重现、用户隔离和注销清理已通过真实 PostgreSQL + Engine 测试；调频事务及计数/权重回放、基础词条所有权和隔离测试通过，禁用/置顶/减半/线性/提升/强制置顶已验证实际目标排名；基础候选删除、单字保护、用户隔离和删除记录重建已通过 HTTP + 原生测试；英文/五笔/简拼/双拼调频及删除已通过原生 HTTP 测试；完整状态 NDJSON 导出已实现，恢复仍待实现 |
