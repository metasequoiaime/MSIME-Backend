...
| 拼音、五笔、英文、快捷短语词库 | 查询、增删改、分页、带权重文本导入导出、纯汉字注音导入；隔离基础词库与用户覆盖 | 四类用户词库 CRUD、分页、事务 TSV 导入导出及增量变更已通过真实 PostgreSQL + Engine 测试；补齐合并基础词条的管理目录分页，已验证全拼/双拼编码、前缀查询、个人覆盖、删除及隔离；基础词条的显式管理编辑能力仍需核对补齐；纯汉字词组注音导入已通过多音字和事务测试；个人覆盖和删除记录已通过 Engine 原生回放进入合并候选，全拼/双拼/五笔/英文/短语用户隔离测试通过；完整状态导出和原子恢复已实现，待生产验收 |
